namespace fourHorsemen_Online_Video_Game_Database.Services
{
    ...
    public class FunFactsService
    {
        ...
        private static readonly List<FactCategory> _categorizedFacts = GetCategorizedFacts();
        private static readonly List<string> _allFacts = _categorizedFacts.SelectMany(c => c.Facts).ToList();
        private static readonly Random _random = new();

        public static List<FactCategory> GetCategorizedFacts()
        {
            return new List<FactCategory>
            {
                new FactCategory
                {
                    Title = "Origins & Odd Histories",
                    Emoji = "🕹️",
                ...
                new FactCategory
                {
                    Title = "Secrets, Easter Eggs & Hidden Content",
                    Emoji = "🥚",
                    Facts = new List<string>
                    {
                ...
                new FactCategory
                {
                    Title = "Famous Fails & Weird Risks",
                    Emoji = "💥",
                    Facts = new List<string>
                    {
                ...
                new FactCategory
                {
                    Title = "Speedruns, Glitches & Alternate Playstyles",
                    Emoji = "⏱️",
                    Facts = new List<string>
                    {
        ...
        {
            return _categorizedFacts.SelectMany(c => c.Facts).ToList();
        }

        public static string GetRandomFact()
        {
            int index = _random.Next(_allFacts.Count);
            return _allFacts[index];
        }
    }
}
